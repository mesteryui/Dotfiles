...
const float DURATION = 0.15; // in seconds (más largo para efecto más suave)
const float MAX_TRAIL_LENGTH = 0.25;
const float THRESHOLD_MIN_DISTANCE = 0.8; // min distance to show trail (units of cursor width)
const float BLUR = 3.0; // blur size in pixels (for antialiasing - más suave)
const float MAX_CURSOR_SPEED = 2.0; // máxima velocidad razonable para detectar ventana desenfocada
const float MAX_TIME_DELTA = 0.5; // máximo delta de tiempo antes de resetear
...
const float SPRING_STIFFNESS = 12.0; // aumentado para mejor curva
const float SPRING_DAMPING = 0.85;

// --- EASING FUNCTIONS ---

// Parametric Spring - más suave y orgánico como kitty
float ease(float x) {
    x = clamp(x, 0.0, 1.0);
...
bool isValidCursorData(vec4 current, vec4 previous, float timeDelta) {
    // Validar que los datos de cursor no sean extremos
    vec2 delta = current.xy - previous.xy;
    float distance = length(delta);
    
    // Si el tiempo delta es muy grande, probablemente la ventana estaba desenfocada
    if (timeDelta > MAX_TIME_DELTA) {
        return false;
    }
    
    // Si el cursor se movió demasiado rápido, probablemente hay un salto de posición
    float speed = distance / max(timeDelta, 0.0001);
    if (speed > MAX_CURSOR_SPEED) {
        return false;
    }
    
    // Validar que el tamaño del cursor sea razonable (> 0)
    if (current.z <= 0.0 || current.w <= 0.0) {
        return false;
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // VALIDACIÓN: Calcular time delta
    float timeDelta = iTime - iTimeCursorChange;
    
    // Solo dibujar trail si los datos son válidos
    if (isValidCursorData(currentCursor, previousCursor, timeDelta)) {
        float minDist = currentCursor.w * THRESHOLD_MIN_DISTANCE;
        float progress = clamp(timeDelta / DURATION, 0.0, 1.0);
    ...
    // Si los datos no son válidos, simplemente mostrar el cursor sin trail
