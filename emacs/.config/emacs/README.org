...
**** Pasvortilo
Paquete que intento que me acepten en el melpa voy a intentar usarlo para ver que errores puedo solventar, es una pequeña interfaz para pass o para gopass, pass es un gestor de contraseña para terminal y gopass es una version moderna de ello, informo que este paquete no soportara todo lo de gopass ya que trata de ser lo más compatible entre pass y gopass, lo hago para sí alguien tiene gopass y no quiere usar pass no tenga que hacerlo.
#+begin_src emacs-lisp 
(use-package pasvortilo
:ensure (:host codeberg :repo "mester/pasvortilo"))
#+end_src
**** Os scratch
...
(setq gptel-model 'gemini-2.5-pro
      gptel-backend (gptel-make-gemini "Gemini" :key (pasvortilo-obtain-password "geminiAPI") :stream t)))
#+end_src
*** Ace window
#+begin_src emacs-lisp 
