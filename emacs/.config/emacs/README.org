...
**** Buffer scratch
#+begin_src emacs-lisp 
(setq initial-scratch-message (format ";; This is `scratch` buffer. Use `%s` for eval and print the result of expression or also you can use `C-x C-e` for eval lisp expressions.\n\n" (propertize
...
(defun os/reload-config ()
  ...
  (sleep-for 0.90)
  (if (equal major-mode 'org-mode) (org-mode))
...
** Paquetes
*** Package lint
Para hacer pruebas con los paquetes y hacer revisiones antes de subirlos a Melpa
#+begin_src emacs-lisp
...
#+end_src
* Carga de archivos
Aqui defino el bloque de codigo que lleva a cabo la carga de todos los modulos que componen esta config
#+name: cargar
#+begin_src emacs-lisp :tangle no
...
(defvar my/enabled-modules
  '(
    ...
    tools       ;; Paquetes y herramientas generales
